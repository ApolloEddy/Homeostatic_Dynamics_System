...
| $P_{NE} > \tau_{\mathrm{ne,high}}$ | 警觉、防御 | 更谨慎表达、更多边界 |
| $P_{NE} < \tau_{\mathrm{ne,low}}$ | 放松、开放 | 更随意表达、更少边界 |
...
| 状态 | 节奏 | 表现 |
| :--- | :--- | :--- |
| $P_{DA} > \tau_{\mathrm{da,high}}$ | 快节奏 | 更快回应、更多话题切换 |
| $P_{DA} < \tau_{\mathrm{da,low}}$ | 慢节奏 | 更慢回应、更少话题切换 |
...
| $P_{5HT} > \tau_{\mathrm{5ht,high}}$ | 结构化对话 | 更有条理、更多逻辑 |
| $P_{5HT} < \tau_{\mathrm{5ht,low}}$ | 自由对话 | 更随意、更多联想 |
...
**参数解释**：
- $w_{adjust\_small}$：小调整值（不展示真实数值）
- $w_{adjust\_tiny}$：微小调整值（不展示真实数值）
...
**参数解释**：
- $w_{energy\_low}$：能量低阈值（不展示真实数值）
- $w_{ne\_high}$：去甲肾上腺素高阈值（不展示真实数值）
- $w_{da\_low}$：多巴胺低阈值（不展示真实数值）
- $w_{social\_high}$：社交饱腹感高阈值（不展示真实数值）
...
**参数解释**：
- $w_{saturation\_high}$：饱和高阈值（不展示真实数值）
- $w_{saturation\_low}$：饱和低阈值（不展示真实数值）
- $w_{saturation\_warning}$：饱和率警告阈值（不展示真实数值）
- $w_{saturation\_critical}$：饱和率严重阈值（不展示真实数值）
