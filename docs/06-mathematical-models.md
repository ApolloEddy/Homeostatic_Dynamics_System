...
- $d_s$：社交需求衰减率（每小时）
- $w_{quality}$：互动质量权重
- $quality$：互动质量（归一化到有界区间：$[q_{\min}, q_{\max}]$）
- $\sigma_{soft}$：软饱和函数
...
- **单调性**：$\sigma_{soft}'(x) > 0$
- **有界性**：$\sigma_{soft}(x) \in (S_{\min}, S_{\max})$
...
**参数解释**：
- $w_{\alpha\_min}$：学习率最小值（不展示真实数值）
- $w_{\alpha\_max}$：学习率最大值（不展示真实数值）
...
本白皮书不展示任何来自代码/配置的真实参数值。参数符号、语义与约束统一收敛在附录：

- `docs/appendix/parameter-symbols.md`
